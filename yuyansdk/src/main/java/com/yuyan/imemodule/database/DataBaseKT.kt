...
abstract class DataBaseKT : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_4_5 = object : Migration(4, 5) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("INSERT INTO skbfun (name, isKeep, position) VALUES ('AiReply', 1, 4)")
                db.execSQL("INSERT INTO skbfun (name, isKeep, position) VALUES ('AiReply', 0, 16)")
            }
        }
        ...
        private fun initPhrasesDb() {  //初始化常用语数据数据
            ...
            if(instance.skbFunDao().getAllMenu().isEmpty()) {
                val skbFuns = listOf(
                    ...
                    SkbFun(name = SkbMenuMode.AiReply.name, isKeep = 1),
                    ...
                    SkbFun(name = SkbMenuMode.AiReply.name, isKeep = 0, position = 16),
                )
