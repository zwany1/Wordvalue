...
import com.yuyan.imemodule.data.SettingsStore
import com.yuyan.imemodule.skill.ContextManager
import com.yuyan.imemodule.skill.EmotionEngine
import com.yuyan.imemodule.skill.EmotionResult
import com.yuyan.imemodule.skill.Example
import com.yuyan.imemodule.skill.GeneratorIntent
import com.yuyan.imemodule.skill.IntentDetector
import com.yuyan.imemodule.skill.ReplyStyle
import com.yuyan.imemodule.skill.SafetyEngine
import com.yuyan.imemodule.skill.SignalDetector
import com.yuyan.imemodule.skill.SignalReport
import com.yuyan.imemodule.skill.StageAssessment
import com.yuyan.imemodule.skill.StageDetector
import com.yuyan.imemodule.skill.StrategyCard
import com.yuyan.imemodule.skill.StrategyLibrary
import com.yuyan.imemodule.skill.TargetProfile
import com.yuyan.imemodule.skill.TemplateReplyEngine
import com.yuyan.imemodule.data.AppDatabase
import com.yuyan.imemodule.skill.SkillRepository
...
class ReplyGenerator(context: Context) {
    ...
    private fun systemPrompt(intent: GeneratorIntent): String = buildString {
        appendLine("你是一个自然的中文聊天回复军师，融合两套方法论：言值七阶段关系推进体系 + Mikey 约会吸引力方法论。风格像兄弟聊天：直接、真诚、偶尔幽默，绝不油腻。")
        appendLine("铁律：话术必须基于对话中已有的信息，绝不虚构用户的经历。")
        appendLine("回答风格：第一句直接给结论，用兄弟的口吻，能短则短，结尾推动行动。")
        when (intent) {
    ...
    private fun buildUserPrompt(
        ...
        val quickstart = retrieveQuickstart(message)
        if (quickstart.isNotEmpty()) {
            appendLine()
            appendLine("高频问题参考（若有匹配请遵循其思路）：")
            quickstart.forEach { appendLine("- Q：${it.first} → A：${it.second}") }
        }
        appendLine()
    ...
    private fun retrieveQuickstart(message: String): List<Pair<String, String>> {
        val raw = runCatching { org.json.JSONObject(repo.datingMaster) }.getOrNull() ?: return emptyList()
        val array = raw.optJSONArray("quickstart") ?: return emptyList()
        val result = mutableListOf<Pair<String, String>>()
        for (i in 0 until array.length()) {
            val item = array.optJSONObject(i) ?: continue
            val q = item.optString("q")
            if (q.any { keyword -> keyword in message }) {
                result.add(q to item.optString("a"))
            }
